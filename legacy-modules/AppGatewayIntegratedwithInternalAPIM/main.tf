# Load SSL Cert from Existing Key Vault

data "azurerm_key_vault" "keyvault0" {
  name                = var.key_vault_name
  resource_group_name = var.key_vault_resource_group_name
}

data "azurerm_key_vault_certificate" "ssl0" {
  name         = var.ssl_certificate_name
  key_vault_id = data.azurerm_key_vault.keyvault0.id
}

# # Get Managed Identity ID

data "azurerm_user_assigned_identity" "mi0" {
  name                = var.identity_identity_ids
  resource_group_name = var.user_assigned_identity_resource_group_name
}


resource "azurerm_application_gateway" "appgateway0" {
  ...
  identity {
    identity_ids = [data.azurerm_user_assigned_identity.mi0.id]
  }

  ssl_certificate {
    name                = var.ssl_certificate_name
    key_vault_secret_id = data.azurerm_key_vault_certificate.ssl0.secret_id # This is really the SSL ID.
  }
